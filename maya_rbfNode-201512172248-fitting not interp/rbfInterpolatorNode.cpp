...
#define PointAs3Double 1 // or as doubleArray.
...
#ifdef PointAs3Double 	
	aPointX = nAttr.create("pointX", "pointX", MFnNumericData::k3Double); // should be the same type with aExamplePoint	
#else
...
MStatus rbfInterpolatorNode::compute( const MPlug& plug, MDataBlock& data )
{
	...
	if (plug == aValueX)
	{
...
#ifdef PointAs3Double
			// aExamplePoint as k3Double type.
...

		{
			...
			double3 &tmpPointX = hPointX.asDouble3();
			std::vector<double> pointX;
			pointX.resize(3);
			pointX[0] = tmpPointX[0]; pointX[1] = tmpPointX[1]; pointX[2] = tmpPointX[2]; 
#else
			MDataHandle hPointX = data.inputValue(aPointX);
			MObject oDoubleData; 
			oDoubleData = hPointX.data();
			MFnDoubleArrayData fnDoubleDataTrans(oDoubleData, &status);
			CHECK_MSTATUS_AND_RETURN_IT(status);
			std::vector<double> pointX;
			pointX.resize(fnDoubleDataTrans.length());
			for (unsigned int j = 0; j < fnDoubleDataTrans.length(); ++j)
			{
				pointX[j] = fnDoubleDataTrans[j];
			} 
#endif 
			Vector P(cols); 
			int ii = 0; 
			...
			for (unsigned int i = 0; i < rows; ++i)
			{
				double distance = ExampleDistance(pointX, examples[i].point);  
				distance = distance / (maxValue > 0 ? maxValue : 1);
...
double ExampleDistance(const std::vector<double> &e1, const std::vector<double> &e2)
{
	// Euclidean distance 
	double dis = 0.0;
	unsigned int pointSize = e1.size() < e2.size() ? e1.size() : e2.size(); 
...
{
	return ExampleDistance(e1.point, e2.point); 
} 
