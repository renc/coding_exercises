...
// or synchronization should be done by derived Listener, here the second case is considered.
//  
// 
...
//              several times, and get result like: 
//              Test 1 took time: 0.000249115s. 
//				Test 1 took time: 0.000371103s.
// Part B. All operations are finished. 
//		   The result of NFQ, COV, POV after each callback are stored in structure NFQCOVPOVTable.
//		   Please check the structure NFQCOVPOVTable for details.
// print result of NFQCOVPOVTable: 
//0, 0, 0, 0, 100, 0, 0.
//0, 100, 0, 100, 100, 0, 0.
//0, 100, 0, 100, 100, 0, 375.
//0, 100, 375, 100, 100, 375, 375.
//5, 50, 375, 50, 50, 375, 375.
//10, 0, 375, 0, 0, 375, 375.
//10, 0, 375, 0, 0, 375, 525.
//- 15, 0, 0, 0, 0, 0, 150.
//- 15, 0, 0, 0, 0, 0, 0.
...
class MsgStopProcessing : public BasicMsg
{
public:
	MsgType msgType() const override { return MsgType::tStopProcessing; }
};
...
class NFQCOVPOVTable {
public:
	int m_iNFQ = 0;
	double m_fCOVBid = 0, m_fCOVOffer = 0;
	double m_fPOVBidMin = 0, m_fPOVBidMax = 0, m_fPOVOfferMin = 0, m_fPOVOfferMax = 0;
	void print() {
		std::cout << m_iNFQ << ", " << m_fCOVBid << ", " << m_fCOVOffer << ", "
			<< m_fPOVBidMin << ", " << m_fPOVBidMax << ", " << m_fPOVOfferMin << ", " << m_fPOVOfferMax << ".\n";
	}
};
...
public:
	...
	void processMessage();
	void stopImmediate();
	void stopAfterProcessingCurrentMsg();
...
private:
	...
	// for Part B 
	NFQCOVPOVTable m_quantityValueTable;
};
...
void DerivedListener::stopImmediate()
{
	if (m_bStopThread == false)
		m_bStopThread = true;
}
void DerivedListener::stopAfterProcessingCurrentMsg()
{
	std::shared_ptr<MsgStopProcessing> pMsg = std::make_shared<MsgStopProcessing>();
	m_msgQueue.push(pMsg);
}
void DerivedListener::processMessage()
{
	...
	while (!m_bStopThread)
	{
		...
		if (m_msgQueue.try_pop(pMsg) == false) {
			...
			if (pMsg->msgType() == MsgType::tStopProcessing) {
				m_bStopThread = true;
			}
			else if (pMsg->msgType() == MsgType::tInsertOrderRequest)
			{
				...
				// 
				if (pM->m_cSide == 'B') {
					m_quantityValueTable.m_fPOVBidMax += pM->m_fPrice * pM->m_iQuantity;
				}
				else if (pM->m_cSide == 'O') {
					m_quantityValueTable.m_fPOVOfferMax += pM->m_fPrice * pM->m_iQuantity;
				}
				m_quantityValueTable.print();
			}
			else if (pMsg->msgType() == MsgType::tReplaceOrderRequest)
			{
				...
				if (m_mapOrderId2Idx.find(pM->m_iOldId) != m_mapOrderId2Idx.end())
				{
					...
					// to add an new order, its quantity is actully the delta quantity,
					// if later be acknowledged, its quantity should be oldOI.m_iQuantity - oldOI.m_iQuantityFilled + delta quantity.
					OrderInfo newOI(pM->m_iNewId, oldOI.m_cSide, oldOI.m_fPrice, pM->m_iDeltaQuantity);
					...
					if (oldOI.m_cSide == 'B') {
						m_quantityValueTable.m_fPOVBidMax += oldOI.m_fPrice * pM->m_iDeltaQuantity;
					}
					else if (oldOI.m_cSide == 'O') {
						m_quantityValueTable.m_fPOVOfferMax += oldOI.m_fPrice * pM->m_iDeltaQuantity;
					}
					m_quantityValueTable.print();
				}
				else {
			...
			else if (pMsg->msgType() == MsgType::tRequestAcknowledged)
			{
				std::shared_ptr<MsgRequestAcknowledged> pM = std::dynamic_pointer_cast<MsgRequestAcknowledged>(pMsg);
				const int iOrderId = pM->m_iId;
				if (m_mapOrderId2Idx.find(iOrderId) != m_mapOrderId2Idx.end())
				{
					OrderInfo &oi = m_aOrderInfo[m_mapOrderId2Idx[iOrderId]];
					double fAcknowledgedValue = oi.m_fPrice * oi.m_iQuantity;
					if (oi.m_cSide == 'B') {
						m_quantityValueTable.m_fCOVBid += fAcknowledgedValue;
						m_quantityValueTable.m_fPOVBidMin += fAcknowledgedValue;
					}
					else if (oi.m_cSide == 'O') {
						m_quantityValueTable.m_fCOVOffer += fAcknowledgedValue;
						m_quantityValueTable.m_fPOVOfferMin += fAcknowledgedValue;
					}
					if (oi.m_iOldId == -1) {
						// this is an insert order 
					}
					else {
						// this is an replace order
						OrderInfo &oldOI = m_aOrderInfo[m_mapOrderId2Idx[oi.m_iOldId]];
						oi.m_iQuantity = oldOI.m_iQuantity + oi.m_iQuantity;
						oi.m_iQuantityFilled = oldOI.m_iQuantityFilled;
					}
					m_quantityValueTable.print();
				}
			}
			else if (pMsg->msgType() == MsgType::tRequestRejectd)
			{
				std::shared_ptr<MsgRequestRejected> pM = std::dynamic_pointer_cast<MsgRequestRejected>(pMsg);
				const int iOrderId = pM->m_iId;
				if (m_mapOrderId2Idx.find(iOrderId) != m_mapOrderId2Idx.end())
				{
					OrderInfo &oi = m_aOrderInfo[m_mapOrderId2Idx[iOrderId]];
					if (oi.m_cSide == 'B') {
						m_quantityValueTable.m_fPOVBidMax -= oi.m_fPrice * oi.m_iQuantity;
					}
					else if (oi.m_cSide == 'O') {
						m_quantityValueTable.m_fPOVOfferMax -= oi.m_fPrice * oi.m_iQuantity;
					}
					m_quantityValueTable.print();
				}
			}
			else if (pMsg->msgType() == MsgType::tOrderFilled)
			{
				std::shared_ptr<MsgOrderFilled> pM = std::dynamic_pointer_cast<MsgOrderFilled>(pMsg);
				const int iOrderId = pM->m_iId;
				if (m_mapOrderId2Idx.find(iOrderId) != m_mapOrderId2Idx.end())
				{
					const int iQuantityFilled = pM->m_iQuantityFilled;
					OrderInfo &oi = m_aOrderInfo[m_mapOrderId2Idx[iOrderId]];
					oi.m_iQuantityFilled += iQuantityFilled;

					const double fFilledValue = oi.m_fPrice * iQuantityFilled;
					if (oi.m_cSide == 'B') {
						m_quantityValueTable.m_iNFQ += iQuantityFilled;
						m_quantityValueTable.m_fCOVBid -= fFilledValue;
						m_quantityValueTable.m_fPOVBidMin -= fFilledValue;
						m_quantityValueTable.m_fPOVBidMax -= fFilledValue;
					}
					else if (oi.m_cSide == 'O') {
						m_quantityValueTable.m_iNFQ -= iQuantityFilled;
						m_quantityValueTable.m_fCOVOffer -= fFilledValue;
						m_quantityValueTable.m_fPOVOfferMin -= fFilledValue;
						m_quantityValueTable.m_fPOVOfferMax -= fFilledValue;
					}
					m_quantityValueTable.print();
				}
			}
...
int main()
{
	// Test for Part A.
	{
		std::cout << "Test for Part A:\n";
		std::unique_ptr<DerivedListener> pListener(new DerivedListener(10, 2));
	...
	// Test for Part B. 
	{
		...
		pListener->OnOrderFilled(1, 5);
		pListener->OnReplaceOrderRequest(2, 3, 10);
		pListener->OnOrderFilled(2, 25);
		pListener->OnRequestRejected(3);
